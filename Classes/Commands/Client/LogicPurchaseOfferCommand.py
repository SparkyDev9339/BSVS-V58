...
class LogicPurchaseOfferCommand(LogicCommand):
    ...
    def decode(self, calling_instance):
        ...
        fields["Item"] = calling_instance.readDataReference()
    ...
    def execute(self, calling_instance, fields):
        ...
        if fields["Unk1"] <= 1000:
            ...
            for rewardData in Shop['Offers'][OfferIndex]['Rewards']:
                CsvID = rewardData['CsvID']
                if rewardData['ItemType'] == 1:
                    RewardID = 7
                    player_data['Coins'] = player_data['Coins'] + rewardData['Amount']
                if rewardData['ItemType'] == 3:
                    RewardID = 1
                    CsvID[0] = rewardData['CsvID'][0]
                    CsvID[1] = rewardData['CsvID'][1]
                if rewardData['ItemType'] == 4:
                    RewardID = 9
                    CsvID[0] = 29
                    CsvID[1] = rewardData['SkinID']
                    player_data['OwnedSkins'].append(CsvID[1])
                if rewardData['ItemType'] == 16:
                    RewardID = 8
                    player_data['Gems'] = player_data['Gems'] + rewardData['Amount']
                if rewardData['ItemType'] == 19:
                    RewardID = 11
                    CsvID[0] = 52
                    CsvID[1] = rewardData['SkinID']
                    player_data['OwnedPins'].append(CsvID[1])
                item = {'Amount': rewardData['Amount'], 'DataRef': [CsvID[0], CsvID[1]], 'RewardID': RewardID}
        ...
        if fields["Unk1"] >= 1000:
            ...
            item = {'Amount': 1, 'DataRef': [fields['Item'][0], fields['Item'][1]], 'RewardID': RewardID}
            box['Items'].append(item)
            box['Type'] = 100
            player_data["delivery_items"]['Boxes'].append(box)
